...
#       code.
#     + For n-bytes character, the first n-bits are all one's, the n+1 bit
...
#     that's correct. But the second continuation byte does not start with 10,
#     so it is invalid.
...
# @return {Boolean}
def valid_utf8(data)
  beginning, ending = 0, data.size
  cursor = beginning

  while cursor < ending
    num =
      case data[cursor]
      when 0b00000000...0b10000000 then 1
      when 0b11000000...0b11100000 then 2
      when 0b11100000...0b11110000 then 3
      when 0b11110000...0b11111000 then 4
      else return false;
      end

    bytes = data[(cursor + 1)...(cursor + num)]
    return false if bytes.size < num - 1
    return false unless bytes.all? { |byte| (0b10000000...0b11000000).include?(byte) }
