...
#     ["ATL", "SFO"]]. Return ["JFK", "ATL", "JFK", "SFO", "ATL", "SFO"]. Another
...
def find_itinerary(tickets)
  [].tap(&->(result){ _visit_('JFK', tickets.sort.group_by(&:first), result) }).reverse
end

private def _visit_(airport, tickets, result)
  _visit_(tickets[airport].shift[1], tickets, result) until Array(tickets[airport]).empty?
  result << airport
end
