...
def is_self_crossing(x)
  i = 2
  loop do
    return false if x[i].nil?
    break unless x[i] > x[i - 2]
    i += 1
  end

  i += 1
  return false if x[i].nil?
  return true if i == 3 && x[i] >= x[i - 2]
  return true if i == 4 && x[i] >= x[i - 2] - (x[i - 1] == x[i - 3] ? x[i - 4] : 0)
  return true if i >= 5 && x[i] >= x[i - 2] - (x[i - 1] + x[i - 5] >= x[i - 3] ? x[i - 4] : 0)
